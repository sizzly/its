 ...
 ********************************************************************************/
?>
			<input align="left" class="crmbutton small cancel"type="button" value="<?PHP echo getTranslatedString('LBL_BACK_BUTTON');?>" onclick="location.href='index.php?action=index&module=HelpDesk'"/>	
			</td>
			<td></td><td></td>
			<td align="right">
...
if($ticketid != '')
{
	...
	if ($ticket_status != 'Closed' && $ticket_status != '') {
		$ticket_close_link = '<a href=index.php?module=HelpDesk&action=index&fun=close_ticket&ticketid='.$ticketid.' onclick="closeTicket(event,this.href)"><b>'.getTranslatedString('LBL_CLOSE_TICKET').'</b></a>&nbsp;&nbsp;&nbsp;';
	} else {
		$ticket_close_link = '';
	}
				...
				if($filelocationtype == 'I'){
					$list .= '
					   		<tr>
								...
								<td class="dvtCellInfo" colspan="3"><a onclick="downloadFile(event,this.href)"href="index.php?downloadfile=true&fileid='.$fileid.'&filename='.$filename.'&filetype='.$filetype.'&filesize='.$filesize.'&ticketid='.$ticketid.'">'.ltrim($filename,$ticketid.'_').'</a></td>
					   		</tr>';
				}else {
					$list .='<tr>' .
								'<td class ="dvtCellLabel" align="right">'.$attachments_title.'</td>' .
